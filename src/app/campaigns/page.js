    ...
    <div className="min-h-screen px-8">
      ...
      <header className="sticky top-0 z-40 bg-[#0a0f1a]/95 backdrop-blur-md border-b border-white/5 py-4 -mx-8 px-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-white/60 mt-1">Discover trading campaigns and earn rewards</p>
          </div>
      ...
      <div className="pb-4">
        <div className="text-sm text-white/40">
          Showing <span className="text-white">{tokens.length}</span> campaigns terminal
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {tokens.map((token) => {
              ...
              const isExpanded = expanded === token.tokenId;

              return (
                <div key={token.tokenId} className="relative rounded-xl bg-[#0d1520] border border-[#00ff41]/20 hover:border-[#00ff41]/50 transition-all duration-300 p-8 flex flex-col w-[400px]">
                  
                  {/* LIVE Badge - Sağ üst köşe */}
                  ...
                  {/* Token Name & Handle - Ortalanmış */}
                  <div className="text-center mb-5">
                    <h3 className="text-2xl font-bold text-white">
                      {token.tokenName || token.tokenId}
                    </h3>
                    <p className="text-sm text-[#00ff41]/60 mt-1.5">{token.ticker || `@${token.tokenId?.slice(0, 8) || 'token'}`}</p>
                  </div>

                  {/* Reward Box - Yeşil arka plan */}
